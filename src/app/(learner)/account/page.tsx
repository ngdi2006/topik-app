    ...
    return (
        <div className="min-h-screen bg-slate-50">
            ...
            <main className="mx-auto w-full max-w-6xl space-y-6 p-4 sm:p-6 lg:p-8">
                <section className="overflow-hidden rounded-3xl bg-gradient-to-br from-blue-700 via-blue-600 to-indigo-700 p-5 text-white shadow-lg sm:p-7">
                    <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex min-w-0 items-center gap-4">
                            <div className="grid size-16 shrink-0 place-items-center overflow-hidden rounded-2xl bg-white/15 text-xl font-black ring-1 ring-white/30">
                                {profile?.avatar_url ? (
                                    // eslint-disable-next-line @next/next/no-img-element
                                    <img alt="" className="size-full object-cover" src={profile.avatar_url} />
                                ) : initials}
                            </div>
                            <div className="min-w-0"><h1 className="truncate text-2xl font-black">{displayName}</h1><p className="truncate text-sm text-blue-100">{user?.email}</p><span className="mt-2 inline-flex rounded-full bg-white/15 px-2.5 py-1 text-[11px] font-bold uppercase">{role}</span></div>
                        </div>
                        <div className="grid grid-cols-2 gap-2 sm:min-w-72">
                            <div className="rounded-2xl bg-white/10 p-3"><p className="text-xs text-blue-100">Bài thi đã làm</p><p className="mt-1 text-xl font-black">{examHistory.length}</p></div>
                            <div className="rounded-2xl bg-white/10 p-3"><p className="text-xs text-blue-100">Điểm trung bình</p><p className="mt-1 text-xl font-black">{averageScore}%</p></div>
                        </div>
                    </div>
                </section>

                <Tabs defaultValue="overview" className="w-full">
                    <TabsList className="grid h-auto w-full grid-cols-3 rounded-2xl bg-white p-1 shadow-sm ring-1 ring-slate-200">
                        <TabsTrigger className="gap-1.5 rounded-xl py-2.5" value="overview"><UserRound className="size-4" />Tổng quan</TabsTrigger>
                        <TabsTrigger className="gap-1.5 rounded-xl py-2.5" value="security"><LockKeyhole className="size-4" />Bảo mật</TabsTrigger>
                        <TabsTrigger className="gap-1.5 rounded-xl py-2.5" value="history"><History className="size-4" />Lịch sử</TabsTrigger>
                    </TabsList>
                    ...
                    <TabsContent className="mt-5" value="history">
                        <Card><CardHeader><CardTitle className="flex items-center gap-2"><History className="size-5 text-blue-600" />Lịch sử học tập</CardTitle><CardDescription>Theo dõi các bài thi và bài kiểm tra đã hoàn thành.</CardDescription></CardHeader><CardContent className="p-0">
                            {isLoading ? <div className="grid min-h-48 place-items-center"><Loader2 className="size-7 animate-spin text-blue-600" /></div> : history.length === 0 ? <div className="p-12 text-center text-sm text-slate-500">Bạn chưa có lịch sử học tập.</div> : <div className="divide-y">{history.map((record) => {
                                ...
                                return <article className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5" key={record.id}><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-blue-50 px-2 py-0.5 text-[11px] font-bold uppercase text-blue-700">{record.type === 'exam' ? 'Bài thi' : 'Mốc học'}</span>{record.exams?.level ? <span className="text-xs text-slate-500">{record.exams.level}</span> : null}</div><h3 className="mt-2 truncate font-semibold text-slate-900">{record.exams?.title || 'Bài kiểm tra'}</h3><div className="mt-2 flex flex-wrap gap-3 text-xs text-slate-500"><span className="inline-flex items-center gap-1"><CalendarDays className="size-3.5" />{new Date(record.created_at).toLocaleString('vi-VN')}</span>{record.time_taken ? <span className="inline-flex items-center gap-1"><Clock3 className="size-3.5" />{minutes} phút</span> : null}<span className="inline-flex items-center gap-1 text-emerald-600"><CheckCircle2 className="size-3.5" />{record.total_correct || 0} đúng</span>{typeof record.wrong_count === 'number' ? <span className="inline-flex items-center gap-1 text-red-600"><XCircle className="size-3.5" />{record.wrong_count} sai</span> : null}</div></div><div className="flex shrink-0 items-center gap-2 rounded-2xl bg-slate-50 px-4 py-3"><Award className="size-5 text-amber-500" /><div><p className="text-[10px] font-semibold uppercase text-slate-400">Kết quả</p><p className="text-xl font-black text-slate-900">{record.score}{record.type === 'exam' ? '%' : ' điểm'}</p></div></div></article>
                            })}</div>}
                        </CardContent></Card>
                    </TabsContent>
                </Tabs>
