...
import { BarChart3, BookOpen, BookText, ChevronDown, FileText, GraduationCap, Languages, LayoutDashboard, Mic, Phone, RotateCcw, Target, Trophy, UserRound, X } from "lucide-react"
import { UserNav } from "@/components/shared/UserNav"
import { LEARNING_CLUSTER_LOCKED } from "@/lib/learner-feature-flags"

const learningItems = [["E-Learning", "/elearning", GraduationCap], ["Giáo trình 2025", "/textbooks", BookOpen], ["Học từ vựng", "/textbooks/learning/vocabulary", Languages], ["Ngữ pháp", "/textbooks/learning/grammar", BookText]] as const

function LockedLearningItems({ mobile = false }: { mobile?: boolean }) {
    return <div className="space-y-1" aria-label="Các chức năng đang phát triển">
        {learningItems.map(([label, , Icon]) => <div className={`flex w-full items-center gap-3 rounded-xl opacity-70 ${mobile ? "h-10 px-3 text-[13px] font-bold text-slate-500" : "min-h-10 px-4 py-2 text-sm font-medium text-blue-100"}`} key={label} title="Đang phát triển">
            <span className={mobile ? "grid size-8 place-items-center rounded-lg bg-slate-100 text-slate-500" : "contents"}><Icon className="size-4 shrink-0" /></span>
            <span className="min-w-0 flex-1">
                {label} <span className={mobile ? "text-[10px] text-amber-700" : "text-[10px] text-blue-100"}>(Đang phát triển)</span>
            </span>
        </div>)}
    </div>
}
...
function SidebarContent({ onNavigate }: { onNavigate?: () => void }) {
    ...
    return (
        <>
            ...
            <nav aria-label="Menu học viên" className="scrollbar-hidden flex w-[calc(100%+20px)] flex-1 flex-col gap-1 overflow-y-scroll pb-1 pl-3 pr-8 pt-4">
                ...
                {LEARNING_CLUSTER_LOCKED ? <LockedLearningItems /> : learningItems.map(([label, href, Icon]) => {
    ...
    return (
        <>
            ...
            <aside aria-hidden={!mobileOpen} aria-label="Menu điều hướng" className={`fixed inset-y-0 left-0 z-[60] flex w-[min(90vw,360px)] flex-col overflow-hidden rounded-tr-[30px] border-l-[6px] border-t-4 border-blue-600 bg-blue-600 text-slate-800 shadow-[20px_0_55px_rgba(15,36,80,0.26)] transition-transform duration-300 md:hidden ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`} inert={!mobileOpen}>
                ...
                <nav className="scrollbar-hidden relative flex min-h-0 w-[calc(100%+20px)] min-w-0 flex-1 flex-col gap-1.5 overflow-x-hidden overflow-y-scroll bg-white py-4 pl-4 pr-9">
                    ...
                    <div className="order-3 mt-3 space-y-1 border-t border-slate-100 pt-3">
                        ...
                        {LEARNING_CLUSTER_LOCKED ? <LockedLearningItems mobile /> : learningItems.map(([label, href, Icon]) => {
                            const active = href === "/textbooks" ? pathname === href : pathname.startsWith(String(href))
